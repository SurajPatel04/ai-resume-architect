...
- Engineered a real-time streaming layer over Server-Sent Events, AWS S3, and a TTS pipeline, achieving a time-to-first-audio of approximately 2 seconds and delivering token-by-token responses to clients.
...
- Integrated the Razorpay payment gateway for secure transactions, implementing custom backend logic for discount coupons to streamline the payment process.
...
- Implemented Redis semantic caching for repeated queries, managed conversation memory with LangGraph, and deployed on GCP using Docker and GitHub Actions CI/CD for seamless updates.
